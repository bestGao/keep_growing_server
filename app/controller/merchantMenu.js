...
class MerchantMenuController extends Controller {
  async index() {
    ...
    try {
      const res = await ctx.service.merchantMenu.query({
        merchantUserId,
      });
  ...
  async page() {
    ...
    try {
      const { count, rows } = await ctx.service.merchantMenu.query({
        merchantUserId,
        ...query,
        ...otherQuery,
      });
  ...
  async create() {
    ...
    try {
      ...
      const res = await ctx.service.merchantMenu.create({
        merchantUserId,
        name,
        isAdmin,
        status,
  ...
  async destroy() {
    ...
    try {
      const record = await ctx.service.merchantMenu.queryById(id);
      if (!record) {
        ctx.body = {
          status: false,
          code: 404,
      ...
      await ctx.service.merchantMenu.destroy(id);
